...
## Status: Stage 4 — Launch, prepared. Blocked on access this session doesn't have.

Everything in Stage 4 that can be done from the repo is done. What remains needs
the live Podia site, a domain registrar, and an inbox — none of which this
session can reach. **`docs/launch.md` is the runbook**, including a paste-ready
prompt for the one task that has to happen first.

| Stage 4 requirement | State |
|---|---|
| Inventory every Podia URL in circulation | **Blocked** — outbound hosts are denied by proxy policy here, so the live site cannot be crawled. Handoff prompt in `docs/launch.md` §1 |
| Redirect map | Infrastructure built and gated; empty until the inventory exists |
| Podia to a subdomain, apex to the new site | **Blocked** — registrar and Podia access |
| Verify past members reach their archives | **Blocked** — needs a real member login. It is step 2 of the cutover in the runbook, deliberately before the apex moves |
| Analytics | Slot built, off until a token is set |
| Meta tags, favicon, social preview | Done |
| Full pass: mobile, keyboard, reduced motion, contrast | Done |

### What's actually in the repo now

- **Social preview card** at `public/og.png` — 1200×630, 55 KB, generated by
  `npm run build:og` from the *real site CSS in a real browser*, so it cannot
  drift from the brand. A link to this site gets pasted into WhatsApp groups and
  Instagram DMs far more than it gets found by search, so a missing card is a
  grey rectangle at the moment of first impression.
- **Complete Open Graph and Twitter tags** on every page, absolute URLs (a
  relative social image is silently ignored).
- `sitemap-index.xml` (404 excluded) and `robots.txt`.
- **Analytics wired to the privacy policy.** Cloudflare Web Analytics is
  cookieless, so the policy can keep saying there is no cookie banner because
  there is nothing to consent to. The privacy page *reads the setting* and
  changes its own wording — verified in both directions, so the policy cannot
  drift from what the site does.
- **Redirect map** with a reviewable JSON source carrying a `note` per rule, a
  generator, and a gate that **fails if a rule points at a page that does not
  exist** — a dangling rule would turn a working bookmark into a 404, which is
  the exact failure the map exists to prevent. Verified by adding a broken rule
...
Fails today on exactly three things, which is correct:

```
BLOCKER: provisional copy is still published
BLOCKER: forms have no handler
BLOCKER: _redirects has no rules — every Podia URL in circulation must be inventoried first
```
...
coordinates, so it read points well inside the button and reported ~1.1:1 for
every candidate — including ones with an obviously dark rim. It nearly led to
abandoning the design as impossible. The gate now renders at 1x so screenshot
pixels and layout pixels are the same unit. If a measurement stops responding
